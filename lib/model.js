...
class Model {
  ...
  static bulkCreate(records, options = {}) {
    ...
    const instances = records.map(values => new this(values, { isNewRecord: true, include: options.include }));
    ...
    const recursiveBulkCreate = (instances, options) => {
      ...
      return Promise.try(() => {
        // Run before hook
        if (options.hooks) {
          return model.hooks.run('beforeBulkCreate', instances, options);
        }
      }).then(() => {
        // Validate
        if (options.validate) {
        ...
        // Run after hook
        if (options.hooks) {
          return model.hooks.run('afterBulkCreate', instances, options);
        }
      }).then(() => instances);
    };
