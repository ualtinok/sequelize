...
const debug = logger.debugContext('sql:memsql');
...
class Query extends AbstractQuery {
  ...
  run(sql, parameters) {
    ...
    return Promise.resolve(
      connection.query(this.sql, parameters)
        ...
        .catch(err => {
          // MemSQL automatically rolls-back transactions in the event of a deadlock
  ...
  logWarnings(results) {
    return this.run('SHOW WARNINGS').then(warningResults => {
      const warningMessage = `MemSQL Warnings (${this.connection.uuid
      || 'default'}): `;
      const messages = [];
